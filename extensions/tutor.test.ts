...
describe('buildTutorPrompt', () => {
  test('is an in-place workflow hint, not a generic lesson', () => {
    ...
    expect(prompt).toContain('First identify the failure mode')
    expect(prompt).toContain('fake validation')
    expect(prompt).toContain('wrong audience')
    expect(prompt).toContain('If the user is angry')
    expect(prompt).toContain('Return exactly five top-level sections')
    expect(prompt).toContain('If you include substeps, use bullets')
  ...
  test('handles realistic build tasks and stays read-only', () => {
    ...
    expect(prompt).toContain('small next move')
