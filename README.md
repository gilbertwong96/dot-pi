...
| `notify.ts`              | Desktop notification when work completes                                               | `KITTY_WINDOW_ID` (Kitty), `TERM_PROGRAM` (iTerm2), or default OSC 777 (Ghostty, WezTerm, foot, urxvt) |
...
| `vision-fallback.ts`    | Client-side vision handoff: analyze images via a vision model for text-only models    | Vision-capable fallback models (e.g. `ollama-cloud/kimi-k2.7-code`, `minimax/MiniMax-M3`)              |
